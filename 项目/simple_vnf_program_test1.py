...
def deploy_sflow():
    ...
    if add_or_del == 'deploy':
        shell = "sudo ifconfig s1 192.168.0.1/24 \n" + \
                "sudo ovs-vsctl -- --id=@sflow create sFlow agent=s1 target=\\\"172.24.1.181:6343\\\"  header=128  sampling=64 polling=1 -- set bridge s1 sflow=@sflow \n" + \
                "sudo ifconfig s2 192.168.0.2/24 \n" + \
                "sudo ovs-vsctl -- --id=@sflow create sFlow agent=s2 target=\\\"172.24.1.181:6343\\\"  header=128  sampling=64 polling=1 -- set bridge s2 sflow=@sflow \n" + \
                "sudo ifconfig s3 192.168.0.3/24 \n" + \
                "sudo ovs-vsctl -- --id=@sflow create sFlow agent=s3 target=\\\"172.24.1.181:6343\\\"  header=128  sampling=64 polling=1 -- set bridge s3 sflow=@sflow \n" + \
                "sudo ifconfig s4 192.168.0.4/24 \n" + \
                "sudo ovs-vsctl -- --id=@sflow create sFlow agent=s4 target=\\\"172.24.1.181:6343\\\"  header=128  sampling=64 polling=1 -- set bridge s4 sflow=@sflow \n"
        print(shell)
        subprocess.getstatusoutput(shell)
...
if __name__ == '__main__':
    ...
    if topo_choice == 'topo1':
        ...
        with open('simple_topo2_program_test1.json','r') as f:
            topo = json.load(f)
    run(topo['networks'])
